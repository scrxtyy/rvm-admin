...
<nav aria-label="secondary" x-data="{ open: false }"
    ...
    @role('employee')
        <div class="relative">
            @php
                $id = Auth::user()->id;
                $notifications = App\Models\Notifications::where('sender_id', '=', $id)->where('isread', '=', 0)->count();
            @endphp
            <a href="{{url('/notifs/'. $id)}}">
                <div>
                Unread <span id= "notifCount" class="relative px-2 p-1 rounded bg-green-400 focus:outline-none hover:bg-green-500 active:bg-green-700 dark:text-gray-200">
                    {{$notifications}}</span>
                </div>
            </a>
        ...
        <script>
          ...
          // Enable pusher logging - don't include this in production  
            // $(document).ready(function() {
              ...
              var channel = pusher.subscribe('update-count');
